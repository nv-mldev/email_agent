...
def is_attachment_supported(attachment):
    ...
    if attachment.name and attachment.name.lower().endswith(SUPPORTED_FILE_EXTENSIONS):
        logger.info(
            "Processing file '%s' based on extension fallback.", attachment.name
        )
        return True
    return False
...
class EmailParserServiceAsync:
    ...
    async def process_message_async(self, message_body: dict):
        ...
        async with GraphClient() as graph_client:
            async with AzureBlobStorageClient() as blob_client:
                with closing(next(get_db())) as db:
                    ...
                    if not log_entry:
                        raise Exception(
                            f"Failed to find log entry for DB ID {db_log_id}"
                        )
                    ...
                    try:
                        ...
                        message = await graph_client.get_message_details(
                            graph_message_id
                        )
                        if not message:
                        ...
                        if message.has_attachments:
                            attachments_meta = (
                                await graph_client.get_attachments_metadata(
                                    graph_message_id
                                )
                            )
